...
def update_test_limit(test_method: Any, test_limit: int) -> None:
    ...
    setattr(
        test_method, PROVIDER_TEST_LIMIT_ATTR_NAME, max(
            existing_test_limit, test_limit)
    )
    ...
    for member_name, member in dct.items():
        ...
        if provider_data is not None:
            ...
            for description, data in (
                ...
                if isinstance(data, dict):
                    new_test.__doc__ = f"{member_name}({', '.join('%s=%r' % (k, v) for k, v in data.items())})"
                else:
                    new_test.__doc__ = f"{member_name}{data}"
